...
\frame{
    ...
    \begin{itemize}
        ...
        \item Frame $L$ je \emph{kompaktní} pokud ke každému $M\subseteq L$, t.ž. $\bigvee M = 1$, existuje konečné $F\subseteq M$, t.ž. $\bigvee F = 1$.
    \end{itemize}
}
...
\frame{
    ...
    \begin{itemize}
        \item Element Stoneova framu je $\kappa$--\emph{generovaný} pokud je supremem množiny komplementárních prvků kardinality menší než $\kappa$.
        ...
        \item Frameový homomorfismus $f$ mezi Stoneovými framy je $\kappa$--\emph{bazicky úplný} pokud platí, že
        \begin{align*}
            f(a^*) = f(a)^*
        \end{align*}
...
\frame{
    ...
    \begin{diagram}[row sep=0.7cm, ampersand replacement=\&, scale = 0.7, transform canvas={scale=0.7}]
        ...
        \ExtrStoneFrm
            \ar[yshift=0.2em]{r}{\visible<2>{\scalebox{1.5}{\BcI}}}
            \ar{u}{\InclUp}
        \& \ComplBool
            \ar{u}{\InclUp}
            \ar[yshift=-0.2em]{l}{\visible<2>{\scalebox{1.5}\JI}}
    \end{diagram}
    \end{column}
    \end{columns}
}
...
\frame{
    ...
    Pro frameové homomorfismy máme zobrazení:
    $$(\Bo f)(x) = f(x)^{**}.$$
...
\frame{
    ...
    \begin{itemize}
        \item Nechť existuje $f\colon L\to M$ na frameový homomorfismus, t.ž. $f(x) = 0$ implikuje $x=0$. Pak říkáme, že $M$ je \emph{hustý v} $L$.
    ...
    \begin{theorem}
        Pro kompaktní metrizovatelný prostor neexistuje netriviální frame v něm superhustý.

        % \smallskip
        % Neboli: Kompaktifikace je metrizovatelná jen tehdy, když už původní frame byl kompaktní a metrizovatelný.
    \end{theorem}
}
