...
\num\label{p:frameOfIdeals} {\bf The frame of ideals.} Let $L$ be a join--semilattice with 0. Denote $\J L$ to be the set of all ideals of $L$. We will show that $\J L$ ordered by inclusion is a frame. An intersection of two ideals is again an ideal, hence finite meets exist.
...
\begin{blockProp*}{Conclusion}
    The set $\J L$ with joins as defined in (\oldref{e:idealJoin}) and meets defined as intersection is a compact frame.
\end{blockProp*}
...
\num\label{p:compactificationFunctor} {\bf The functor \C{}.} Now, we are ready to define a functor from the category of completely regular frames to the category of compact regular frames. Denote
...
\noindent for any frame homomorphism $f\colon L\to M$ between completely regular frames (because $\comp_M(\C f)(I) = \bigvee (\downset f[I]) = \bigvee f[I] = f[\bigvee I] = f \comp_M(I)$).
