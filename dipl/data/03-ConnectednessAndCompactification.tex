...
\begin{definition}
    We say that a frame $L$ is \DEF{disconnected}[ frame] if $L$ contains non-trivial elements $a$ and $b$ (that is, different from the top and the bottom of $L$) such that $a\vee b = 1$ and $a\wedge b = 0$. If a frame is not disconnected, we say that it is \DEF{connected}[ frame].
\end{definition}
...
\begin{definition}
    ...
    \vspace{0.8em}
    A frame $L$ is said to be \DEF{zero--dimensional}[ity] if every element is a join of complemented elements.
\end{definition}
...
\begin{definition}
    A frame $L$ is \DEF{compact}[ frame] if for every \DEF{cover}, that is a subset $C \subseteq L$ such that $\bigvee C = 1$, there exists a finite subcover (a finite $F \subseteq C$ such that $\bigvee F = 1$).
\end{definition}
...
\begin{proof}
    ...
    \noindent By the same argument, $(f_b\wedge f_a^*)\vee a = 1$. Further, $(f_b\wedge f_a^*)\wedge (f_a\wedge f_b^*) = 0$, therefore $L$ is normal.
\end{proof}
...
\begin{theorem*}\label{p:universalCompactification}
    The functor \C{} and the natural transformation $\comp$ provides a coreflection of the category of completely regular frames onto the category of compact regular frames.
...
\begin{theorem*}\label{p:HLDuality}
    The category of compact regular frames is equivalent to the dual of the category of compact regular spaces.
