...
Compactness, connectedness and variants of disconnectedness are commonly studied in topological spaces. In this chapter, we will prove some results which are well known from Set theoretical topology and we will prove them in the context of a Point-free topology.
...
\begin{observation}\label{p:disconnectednessEquivalently}
    ...
    \begin{enumerate}
        ...
        \item There exists a non-trivial complemented element in $L$.
...
\begin{proof}
    \begin{enumerate}
        \item For every $a$ complemented, we have $a \crbelow a$. Therefore, by zero--dimensionality
...
\num {\bf A frame of ideals.} Let $L$ be a frame. Denote $\J L$ to be the set of all ideals of $L$. We will show that $\J L$ ordered by inclusion is a frame. Meet of two ideals is simply its intersection (an intersection of ideals is again an ideal -- TODO).
    ...
    The set defined this way is again an ideal and it is the supremum of $\Set{ I_i | i\in J}$. Also for any ideals $J$ and $I_i$ of $L$, for $i \in J$, the following equality holds
    $$(\bigvee_i I_i) \cap J = \bigvee_i (I_i \cap J).$$

    \noindent The $\supseteq$ inclusion is trivial, for the other inclusion take $x\in J$ such that $x = \bigvee F$ for some finite $F \subseteq \bigcup_i I_i$. Then $F \subseteq J$ (as $J$ is an ideal) and also $F \subseteq \bigcup_i (I_i \cap J)$. We get, $x \in \bigvee_i (I_i \cap J)$. Thus, \J{} is a frame.
...
TODO conclusion? $\R L$ together with joins as defined at (\oldref{e:idealJoin}) and intersection as meets is a frame. 
...
\begin{proof}
    First, interpolate $a \crbelow x\crbelow y\crbelow b$. By~\ref{p:crbellowProperties}, $x^*\crbelow a^*$ and $x^*\in \sigma(a^*)\subseteq \sigma(a)^*$. Since $y\in \sigma(b)$ and $x^*\vee y = 1$, we have $\sigma(a)^*\vee \sigma(b) = 1_{\R L}$.
\end{proof}
...
\num {\bf The functor \C{}.} Now, we are ready to define a functor from the category of completely regular frames to the category of compact regular frames. Denote
    $$\DEFSYM{compactificationFunctor}{\C}\colon \CRegFrm \to \RegKFrm$$

    \noindent to be the following two mappings. On objects:
    $$\C(A) = \R A,$$
    ...
    And we can conclude that $\C f$ is a frame homomorphism. As a result, we have the following
...
\num For a completely regular frame $L$, define \DEFSYM{compactification}{$\comp_L$}$\colon \C L\to L$ as follows:
    ...
    The mapping $\comp_L$ is in familiar connection with $\sigma$:
    ...
    Consequently, $\comp_L$ is a frame homomorphism and $\sigma$ is a localic map. Observe $\comp_L$ is also dense, $\bigvee I = 0$ implies $I = \{0\}$.


\num We see that $\comp_*\colon \C{} \to \Id$ is a natural transformation, because the following diagrams commute
...
\begin{center}
    (since $\comp_M(\C f)(I) = \bigvee (\downset f[I]) = \bigvee f[I] = f[\bigvee I] = f \comp_M(I)$)
\end{center}
...
\begin{proof}
    ...
    Every ideal of $L$ corresponds to some $\sigma(a)$ and $\comp_L$ is an one-one homomorphism, thus $\comp_L$ is an isomorphism.
\end{proof}

Moreover, \C{} is a reflection of categories with unit of adjunction $\comp_L$.
...
\end{lemma}
\begin{proof}
    Let $U = \open(a)$, for some $a$, then $\bigwedge (\open(a)\cap S) = \bigwedge \Set{ a\to s | s \in S } = a\to \bigwedge S$. And $\bigwedge S = 0$, since $S$ is dense in $L$ (and includes $0_L$). Hence $\closure{U\cap S} = \upset (\bigwedge (U\cap S)) = \upset(a\to 0) = \clos(a^*) = \closure{U}$.
\end{proof}
...
\end{proposition}
\begin{proof}
    Let $U$ be an open sublocale of $\C L$. Take $V = U\cap L$ open sublocale of $L$. We know that $\closure V^L = \closure{U\cap L}^L = \closure{U\cap L}\cap L = \closure U\cap L$. Moreover $\closure V^L$ is clopen in $L$.
