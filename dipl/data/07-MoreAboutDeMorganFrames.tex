...
\begin{definition}
    Let $f\colon L\to S$ be a dense onto frame homomorphism. We say that $S$ is \DEF{superdense} in $L$ or that $f$ is \emph{superdense} if, for any compact regular $K$ and any frame homomorphism $h\colon K\to S$, there exists a frame homomorphism $\tilde h\colon K\to L$ such that the following diagram commutes
