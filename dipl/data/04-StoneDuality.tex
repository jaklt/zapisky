...
From the fact that a homomorphic image of a complemented element is a complemented element and since join or meet of two complemented elements is again complemented (to see that, let $a$ and $b$ be two complemented elements, then
    \begin{align*}
        ...
        (a\wedge b) \wedge (a^c \vee b^c)
            ...
            = 0 \vee 0 = 0;
    \end{align*}
and similarly $a^c\wedge b^c$ is complement of $a\vee b$.), one can see that $\Bc f$ is well--defined.
...
\begin{proposition}
    Let $H$ be a Heyting semilattice, then $\Bo H$ is a Boolean algebra with joins and meets defined
...
\end{lemma}
\begin{proof}
    From extremally disconnectedness, the following holds
    \begin{align*}
        f(a^{**} \vee a^*) & = f(a^{**}) \vee f(a^*) = 1, \text{ and} \\
        f(a^{**} \wedge a^*) & = f(a^{**}) \wedge f(a^*) = 0.
    \end{align*}
    Hence $f(a^{**})$ is complement to $f(a^*)$ and we have $f(a^*) = f(a^{**})^*$.
\end{proof}
% TODO more verbose?
...
TODO homomorphisms preserving $\kappa$--completeness!
...
\begin{tikzpicture}
    ...
    % Categories captions
    \node at (-1, 1.2) {$\sigma$--\categoryStyle{BasDiscStoneFrm}};
    \node at (1, 0.9) {$\vdots$};
    \node at (-1,-0.2) {$\kappa$--\categoryStyle{BasDiscStoneFrm}};
    \node at (1,-0.6) {$\vdots$};
...
\noindent Where $\kappa$--\ComplBool{} denotes the category of $\kappa$--complete Boolean algebras and $\kappa$--complete Boolean homomorphisms and $\kappa$--\categoryStyle{BaDiscStoneFrm} denotes the category of $\kappa$--basically disconnected Stone frames and corresponding frame homomorphisms.
