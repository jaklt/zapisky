...
Stone duality is a pioneer work by/classical result of ... Stone, it is duality of two categories the category of Boolean algebras and Boolean homomorphisms and the category of compact, Hausdorff, zero--dimensional topological spaces and continuous maps. At that time, without the language of Category Theory.

The duality means ... contravariant

TODO a lot nicer: The duality at that time was a little surprise, because it showed similarities between algebraic structure with stg. witch was thought has no algebraic structure at all.

In this chapter, we will first reconstruct classical Stone duality for frames and as the Category of frames is contravariant position to category of topological spaces, the whole construction will be covariant. Hence the name Stone correspondence. And in the second part, we will reconstruct the classical Stone duality from this correspondence.
...
One can check, the whole correspondence between Stone frames and Boolean algebras is given constructively. Therefore the necessity of choice principle needed in classical Stone duality between Stone spaces and Boolean algebras is only to show that spaces in the duality have enough points.

However, in the proof of the fact that compact regular frames are completely regular (\ref{??}) we construct interpolative relation $\crbelow$ with unavoidable usage of Axiom of Countable Dependent Choice (CDC). We actually do not have to worry about that, by Banaschewski and Pultr [TODO], we can avoid using CDC at all by working with strongly regular frames instead of completely regular. The whole construction can then be made constructive, even in sense of Topos Theory.

Strongly regular frames are frames, in which for every element $x$ the following holds
$$ x = \bigvee \Set{ y | y\;(\rbelow)_o\;x }. $$

Where $(\rbelow)_o$ is the largest interpolative relation contained in $\rbelow$. Such relation can be constructed as the union of all interpolative relations contained in $\rbelow$.
