...
For the rest of this section, let $\kappa$ be some fixed cardinal.

\begin{observation}
    Let $f\colon A \to B$ be a $\kappa$--complete Boolean homomorphism and let $S \subseteq A$, $|S| \leq \kappa$ and denote $I$ to be the ideal generated by $\Set{ \downset s | s \in S}$ then
    $$(\J f)(I^*) = (\J f)(I)^*.$$
\end{observation}
\begin{proof}
    \begin{align*}
        (\J f)(I^*) &= (\J f)(\downset (\bigvee I)^*) = \downset f((\bigvee I)^*) \\
                ...
                &= (\J f)(I)^*
    \end{align*}
    We could use $\kappa$--completeness in the third step because $I = \bigvee \Set{ \downset s | s \in S }$ and $|S| \leq \kappa$.
\end{proof}
% TODO more elegant

\begin{definition}
    Let $L$ be a Stone frame and let $s$ be any element of $L$. We say $s$ is a \DEF{$\kappa$--generated} if $s$ is a supremum of at most $\kappa$ complemented elements.
...
In other words the last observation states that \J{} sends any $\kappa$--complete Boolean homomorphism to $\kappa$--basically complete frame homomorphism.
...
\begin{proof}
    ...
    Now suppose $m \leq a$ for all $a \in S$. Then $m\wedge M = (m^c\vee \bigvee \Set{b^c | b \in S})^c = (m^c)^c = m$. Hence $m \leq M$, which finishes the proof.
\end{proof}

\begin{conclusion}
    Any Boolean homomorphism preserving all $\kappa$--meets (or $\kappa$--joins) is $\kappa$--complete.
\end{conclusion}

\begin{lemma}
    Let $f\colon L \to M$ be a $\kappa$--basically complete frame homomorphism, then $\Bc f$ is a $\kappa$--complete Boolean homomorphism.
\end{lemma}
\begin{proof}
    For an arbitrary $A \subset \Bc L$, $|A| \leq \kappa$, we have
    \begin{align*}
    ...
    \end{align*}
    Therefore by the previous Conclusion $\Bc f$ is $\kappa$--complete.
\end{proof}
...
\end{definition}
% TODO rewrite using `$\kappa$--generated' terminology

\begin{definition}
    A lattice is \DEF{$\kappa$--complete}, for some cardinal $\kappa$, if any of its subset of cardinality at most $\kappa$ has a supremum.
\end{definition}
...
\end{lemma}
\begin{proof}
    By the lemma \ref{p:complIdeal} we know that complemented ideals are precisely principal ideals. To prove the lemma we will show that for any $M \subseteq B$, $|M| \leq \kappa$ and $I = \bigvee \Set{ \downset a | a \in M }$, $I^{**} \vee I^* = 1_{\J B}$.

    Define $m = \bigvee M$ and $J = \downset m$. Observe that $I^{**} = J$: Trivially $I^{**} \subseteq J^{**} = J$ and $J \subseteq I^{**}$ follows from
    $$ I^* = \bigcup \Set{ \downset a | \downset a \cap I = \{0\} } = \bigcup \Set{ \downset a | a \wedge m = 0 } = (\downset m)^* = J^*.$$

    Consequently, $I^{**} \vee I^* = J \vee J^* = \downset m \vee \downset m^c = 1_{\J B}$.
\end{proof}

\num From the previous two Lemmas we see that restriction of classical Stone correspondence to subcategories of $\kappa$--complete Boolean algebras on one side and $\kappa$--basically disconnected Stone frames on the other (without any restriction to morphisms) is still a duality of categories. However, we can strengthen the result to get the following
% TODO improve the last sentence 
...
\begin{proof}
    ...
    \begin{align*}
        \bigsqcup i_B(b_j) &= \bigsqcup \downset b_j = (\bigvee \downset b_j)^{**} = (\downset (\bigvee (\bigvee \downset b_j))^*)^* = (\downset (\bigvee b_j)^*)^* \\ 
        &= \downset (\bigvee b_j)^{**} = \downset (\bigvee b_j) = i_B(\bigvee b_j).
    \end{align*}
