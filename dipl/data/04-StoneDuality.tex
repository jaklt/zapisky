...
From previous observations, we know that $v_L$ is an isomorphism of $L$ and $\JO\BcO(L)$, also by direct computation, for any homomorphism of Stone frames $f\colon L \to M$, the following diagram commutes
...
\noindent (for any $I\in \JO\BcO(L)$, $(v_M\cdot\JO\BcO(f)) (I) = \bigvee (\downset f[I]) = \bigvee f[I] = f(\bigvee I) = f\,v_L (I)$). Again, as conclusion of previous paragraphs we obtain
...
\num Let $B$ be a Boolean algebra, define
    ...
    $$
\noindent where \DEFSYM{OI}{$O_I$} is the set $\Set{ F \in X_B | F \cap I \neq \emptyset }$. The pair $(X_B, \tau_B)$ is a topological space:
...
\end{lemma}
\begin{proof}
    Compactness follows directly from compactness of $\JO B$. Notice, complemented elements are of the form $O_{\downset a}$ for some $a\in B$: $O_{\downset a}\cup O_{\downset a^c} = O_{\downset (a\vee a^c)} = X_B$ and $O_{\downset a}\cap O_{\downset a^c} = O_{\downset (a\wedge a^c)} = \emptyset$. Since every open set is an union of sets of the form $O_{\downset a}$, the $\Sp B$ is zero--dimensional.
...
\noindent From the previous Lemma, we see the definition is sound. We will show that $\Sp f$ is also continuous. Take any $O_I \in \tau_B$, we have
\begin{align*}
...
\num Similarly to the previous section, we obtained an isomorphism of categories. However, the adjoint functors of this isomorphism are contravariant, thus we obtained a duality of two categories instead of a correspondence.

Note that we can make the correspondence, from the previous section, into duality simply by the duality between the category of frames and the category of locales.
