...
\num\label{p:BoolEquivalence} For a Boolean algebra $B$, define \DEFSYM{istar}{$i_B$}$\colon B \to \Bc\J(B)$ as follows
...
\num\label{p:StoneFrmEquivalence} Similarly for $L$, a Stone frame, we have a mapping \DEFSYM{vstar}{$v_L$}$\colon \J\Bc(L) \to L$ defined as
...
We can see that both $v_L$ and $\iota$ are monotone maps, $v_L \iota = \id_L$ (by zero--dimensionality of $L$) and $\id_{\J\Bc(L)} \subseteq \iota v_L$, therefore $v_L$ is the left Galois adjoint to $\iota$, hence $v_L$ preserves all suprema.
