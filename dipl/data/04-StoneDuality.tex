...
\begin{theorem*}\label{p:stonefrmBoolIso}
    Functor \BcO{} is the right adjoint to the functor \JO{} with $i$ as the unit of adjunction and $v$ as the counit. Moreover, \BcO{} and \JO{} constitute an isomorphism of categories \StoneFrm{} and \Bool.
\end{theorem*}
...
\num Let $B$ be a Boolean algebra, define
    ...
    $$
\noindent where \DEFSYM{WI}{$W_I$} is the set $\Set{ F \in X_B | F \cap I \neq \emptyset }$. The pair $(X_B, \tau_B)$ is a topological space:
...
\begin{theorem*}
    Functors $\Omega\circ\BcO$ and \Sp{} are mutually inverse. Thus, categories \Bool{} and $\StoneSp^{\text{op}}$ are isomorphic.\ACP
\end{theorem*}
...
Analogously, the compactification of completely regular frames described in the previous chapter was constructive; its counterpart for topological spaces is equivalent to Boolean Ultrafilter Theorem~\cite{??}.
