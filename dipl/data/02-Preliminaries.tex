...
In this chapter we present a summary of all relevant and well--known facts to point--free topology which we will need in the following chapters.
...
$[0,1]$ is the closed interval of real numbers greater than or equal to 0 and less than or equal to 1, similarly, $[0,\infty)$ is the set of all non-negative real numbers, and $\mathbb Q$ is the set of all rational numbers.
...
A \emph{lattice} is partially ordered set which is both join-- and meet--semilattice. We write $a\vee b$ for the supreum (the \emph{meet}) of $\{a,b\}$ and $a\wedge b$ for the infimum (the \emph{join}).
...
\num A lattice $D$ is \DEF{distributive}[ lattice] if the following equation holds for all $a,b,c\in D$:
\begin{align*}
...
\num Let $A$ and $B$ be two partially ordered sets. Monotone mappings $f\colon A \to B$ and $g\colon B\to A$ are in \DEF{Galois connection} or are \DEF{Galois adjoint} if
...
Then, we say that $f$ is a left Galois adjoint of $g$ and $g$ is a right Galois adjoint of $f$.
...
\num Let $L$ be a lattice with 0. A \emph{pseudocomplement} $a^*$ of an element $a\in L$ is the greatest element $x$ such that
$$ x\wedge a = 0. $$
...
\num Let $L$ be a meet semilattice with 0 and with a binary operation $\to$. Then $L$ is said to be a \DEF{Heyting algebra} if the following holds
...
\begin{proposition*}
    ...
    holds whenever $\bigvee_i a_i$ exists in $H$.
\end{proposition*}
...
A \emph{principal filter} is any filter of $L$ of the form $\upset a = \Set{ x | x\geq a}$.

Similarly, an ideal $I$ is called \emph{prime ideal}, \emph{completely prime ideal} or \DEF{principal ideal} if $I$ is a prime filter, completely prime filter or principal filter of $L\op$.
...
From the Proposition we see that the definition of a maximal filter and a prime filter coincide for Boolean algebras, hence we call such filter \DEF{ultrafilter}.
...
\axiom{\DEF{(AC)}}{Let $X$ be a non-empty partially ordered set such that every non-empty chain has an upper bound. Then $X$ has at least one maximal element.}

\noindent By \DEF{Boolean Ultrafilter Theorem} we mean the following choice principle

\axiom{\DEF{(BUT)}}{Let $B$ be a Boolean algebra and let $F$ be a filter of $B$. Then there exists a ultrafilter extending $F$.}

\noindent This is equivalent to

\axiom{\emph{(BUT')}}{Let $B$ be a Boolean algebra and let $F$ be a filter and let $I$ be an ideal of $B$ such that $F\cap I = \emptyset$. Then there exists a prime filter $G$ extending $F$ such that still $G\cap I=\emptyset$.}

\noindent Finally, the last choice principle we will need to know is \DEF{Axiom of Countable Dependent Choice}

\axiom{\DEF{(CDC)}}{Let $R$ be a binary relation on a set $X$ such that for every $a\in X$ there exists $b\in X$ satisfying $aRb$. Then there exist a countable sequence $(a_i)_{i=1}^\infty$ such that $a_iRa_{i+1}$ for all $i=1,2,\dots$.}
...
A colimit is defined dually to limit.
A \emph{colimit} of a diagram is a constant functor $L\colon \p D\to \p C$ and a natural transformation $l\colon D\natto L$ such that 
for any constant functor $K\colon \p D\to \p C$ and a natural transformation $k\colon D\natto K$ there exists an unique natural transformation $\tilde k\colon L\natto K$ satisfying  $k = \tilde k\,l$.
...
Two categories $\p C,\p D$ are said to be \emph{equivalent}\index{equivalence of categories}, and we write $\p C\cong \p D$, if there exists adjoint functors $F\colon \p C\to \p D$ and $G\colon \p D\to \p C$ with units of adjunction consisting of natural equivalences.
...
\num For a topological space $X$, a continuous map $i\colon \{\star\}\to X$ is identified with a point of $X$. For each such map, we have the associated frame homomorphism $\Omega(i)\colon \Omega X\to \mathbf 2$, where $\mathbf 2$ is the frame $\Omega(\{\star\}) = \{0 < 1\}$. Therefore, it is convenient to define a \emph{point} of a frame $L$ as a frame homomorphism $L\to \mathbf 2$.

Note that every frame homomorphism $p\colon L\to \mathbf 2$ determines a completely prime filter (as $p^{-1}[\{1\}]$), and vice versa. The completely prime filters in topological spaces represents the neighbourhoods of points.

Another way of defining points of a frame is by prime elements.
An element $p\neq 1$ is \DEF{prime}[ element] or \DEF{meet--irreducible}[ element] if $a\wedge b\leq p$ implies $a\leq p$ or $b\leq p$.
Then, the frame homomorphisms $p\colon L\to \mathbf 2$ are in one-one correspondence with prime elements (as $\bigvee p^{-1}[\{0\}]$).
The prime elements of topological space corresponds to the open sets $X\setminus \closure{\{x\}}$.

A frame $L$ is said to be \DEF{spatial}[ frame] or has \emph{enough points} if $L = \Omega(X)$ for some space $X$.
