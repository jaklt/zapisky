...
Then, the frame homomorphisms $L\to \mathbf 2$ are in one-one correspondence with prime elements (as $\bigvee p^{-1}[\{0\}]$).
The prime elements of topological space approximate the open sets $X\setminus \closure{\{x\}}$.

A frame $L$ is said to be \DEF{spatial} or has \DEF{enough points} if $L = \Omega(X)$ for some space $X$.
