...
\begin{block*}{A few comments on notation}
...
The rightmost unary operation is applied first, for example $\downset \bigvee a_i^*$ means $\downset \left(\bigvee_i (a_i^*)\right)$.
\end{block*}
...
A partially ordered set $(A,\leq)$ is said to be \emph{with 1 -- the top} (resp. \emph{with 0 -- the bottom}), if $1\in A$ (resp. $0\in A$) and $1\geq x$ (resp. $0\leq x$) for all $x\in A$.
A partially ordered set with 0 and with 1 is said to be \emph{bounded}.
Denote by $(A,\leq)\op$ the partially ordered set $(A,\leq')$ where $(\leq') = \Set{ (b,a) \in A\times A | a\leq b}$.

% TODO better wording
Let $S$ be a subset of $A$.
We say that $u$ is an upper bound for $S$ if $u \geq x$ for all $x$ of $S$. We say that $s$ is a supremum for $S$ if $s$ is the least upper bound for $S$. We write $s = \bigvee S$.
We say that $l$ is an lower bound for $S$ if $l \leq x$ for all $x$ of $S$. We say that $i$ is a infimum for $S$ if $i$ is the supremum for $S$ in $A\op$, i.e. if $i$ is the greatest lower bound for $S$. We write $i = \bigwedge S$.
...
Then we say that $f$ is a left Galois adjoing and a $g$ is a right Galois adjoing.
...
We say that a category is \emph{(co)complete} if there is a (co)limit for every diagram.
