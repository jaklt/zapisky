...
\begin{definition}
    A lattice is \DEFSYM[complete]{}{$\kappa$--complete}, if any subset of cardinality less than $\kappa$ has a supremum and an infimum. We say a homomorphism is $\kappa$--complete if it preserves all suprema and infima of subsets of cardinality less than $\kappa$.
...
\num The following Lemma will be often handy for computations.
\begin{lemma*}\label{p:idealsFrame}
    Let $B$ be a $\kappa$--complete Boolean algebra and let $I$ be a $\kappa$--generated element of $\JO B$. Then $I^* =\;\downset (\bigvee I)^c$ in $\JO B$. Specially, for $a \in B$, $(\downset a)^* = \downset a^c$.
\end{lemma*}
\begin{proof}
...
\end{lemma}
\begin{proof}
    By Lemma~\ref{p:complIdeal}, we know that complemented ideals are precisely the principal ideals of $B$. Given any $M$ subset of $B$ of cardinality less than $\kappa$, set $I = \bigvee \Set{ \downset a | a \in M }$. We will show that $I^{**} \vee I^* = 1_{\J B}$.
...
\begin{proof}
    ...
    \begin{align*}
        (\JO f)(I^*) &= (\JO f)(\downset (\bigvee I)^*) & \text{(Lemma~\ref{p:idealsFrame})}\\
    ...
    Where the use of $\kappa$--completeness of $f$ in the fourth step is valid because $I$ is $\kappa$--generated. Specially, $I = \bigvee \Set{ \downset s | s \in S }$ for some $S$ a subset of $A$ of the cardinality less then $\kappa$.
\end{proof}
% TODO more elegant

% As we will see after a while, the above Observation characterises morphisms of the image of $\kappa$--complete part of Boolean algebras in Stone correspondence.
...
By Theorem~\ref{p:kappaCompleteThm} we know that there is an isomorphism between the category of $\kappa$--complete Boolean algebras and $\kappa$--complete Boolean homomorphisms and the category of $\kappa$--basically disconnected Stone frames and $\kappa$--basically complete frame homomorphisms. Since, there is no limitation or upper bound for the cardinal $\kappa$ in Theorem~\ref{p:kappaDuality}, we take a look at the part of the correspondence, where $\kappa$ is arbitrary large.

\num It is interesting to see, how the Stone frame part of the correspondence looks like. Take any object $L$ and an element $a\in L$. From zero--dimensionality, we know that $a$ is $\lambda$--generated for some regular cardinal $\lambda$, but $L$ is $\lambda$--basically disconnected, hence

$$a^{**}\vee a^* = 1$$

\noindent holds. Since $a$ has been chosen arbitrarily, we obtain $L$ an extremally disconnected frame. Similarly, any morphism of this part of the duality $f$ with domain $L$ is $\lambda$--basically complete, the following holds
...
Thus, the resulting category is the category of extremally disconnected Stone frames and basically complete frame homomorphisms. We will denote this category \DEF{\ExtrStoneFrm}.

\num On the side of Boolean algebras we have Boolean algebras having arbitrary large joins and Boolean homomorphisms preserving such joins. Thus the observed category is the category of complete Boolean algebras and complete Boolean homomorphisms. We will denote it \DEF{\ComplBool}.

\num\label{p:completePartUnits}Recall, the definitions of $i_B\colon B \to \BcO\JO(B)$ and $v_L\colon \JO\BcO(L) \to L$ are:
    ...
    $$
    \bigsqcup_{i\in I} i_B(a_i) = \bigsqcup_{i\in I} \downset a_i = (\bigvee_{i\in I} \downset a_i)^{**} = \downset (\bigvee_{i\in I} a_i)^{**} = \downset (\bigvee_{i\in I} a_i) = i_B(\bigvee_{i\in I} a_i).
    $$

    \noindent Consequently, $i_B$ is a complete Boolean homomorphism and morphisms of \ComplBool{}.
...
\begin{theorem*}
    Functor \BcI{} is the right adjoint to the functor \JI{} with $i_*$ as unit of adjunction and $v_*$ as counit, and the categories \ExtrStoneFrm{} and \ComplBool{} are isomorphic.
\end{theorem*}
...
\begin{proof}
    ...
    From Lemma~\ref{p:extrDiscPreserv}, we know that compactification preserves extremal disconnectedness and therefore $\J{B}$ is also extremally disconnected. And we know that $\J{B}$ is also a Stone frame by Lemma~\ref{p:JisFunctor}.
\end{proof}

The proof of the previous Proposition unveils an important fact about the complete part of Stone correspondence. It shows that this part is an isomorphism of two subcategories of the category of frames (on Boolean side, without any restriction to frame homomorphisms) and that it is provided by a purely topological construction, by compactification.
