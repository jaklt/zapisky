...
For the rest of this section, let $\kappa$ be some fixed infinite regular cardinal.

\begin{definition}
    A lattice is \DEFSYM[complete]{}{$\kappa$--complete}[ lattice or homomorphism], if any subset of cardinality less than $\kappa$ has a supremum and an infimum. A homomorphism is said to be \emph{$\kappa$--complete} if it preserves all suprema and infima of subsets of cardinality less than $\kappa$.

    Let $L$ be a Stone frame and let $s$ be an element of $L$. We say that $s$ is a \DEFSYM[generated]{}{$\kappa$--generated} if $s = \bigvee S$ for some $S \subseteq \BcO L$ of cardinality less than $\kappa$.
\end{definition}
...
\num\label{p:kappaCompleteBAfromMeets}The following Lemma will be often very useful. We will use it without further referencing.
\begin{lemma*}
...
\num It is an interesting to see, how the Stone frame part of the correspondence looks like. Take any object $L$ and an element $a\in L$. From zero--dimensionality, we know that $a$ is $\lambda$--generated for some regular cardinal $\lambda$, but $L$ is $\lambda$--basically disconnected, hence

$$a^{**}\vee a^* = 1$$

\noindent holds. Since $a$ has been chosen arbitrarily, we obtain $L$ an extremally disconnected frame. Similarly, any morphism of this part of the duality $f$ with domain $L$ is $\lambda$--basically complete, hence the following holds
\begin{align}
    f(a^*) = f(a)^*.\label{e:no0}
\end{align}

\noindent We will call morphisms satisfying (\oldref{e:no0}), for all elements, simply \DEF{basically complete} frame homomorphisms.
...
\begin{theorem*}
    The functor \BcI{} is the right adjoint to the functor \JI{} with $i_*$ as unit of adjunction and $v_*$ as counit, and the categories \ExtrStoneFrm{} and \ComplBool{} are isomorphic.
\end{theorem*}
