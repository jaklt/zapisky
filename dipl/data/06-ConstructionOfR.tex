...
\begin{lemma}
    Let $B$ be a Boolean frame, then $B \cong \Bo\JI(B)$ in \ComplBool.
\end{lemma}
\begin{proof}
    We see $J \in \Bo\JI(B)$ iff $J = J^{**} = \downset \bigvee J$. On the other hand, for $a \in B$: $(\downset a)^{**} = \downset a^{**} = \downset a$ in $\JI B$.
...
\begin{lemma}
    Let $L$ be an extremally disconnected Stone frame, then $L \cong \JI\Bo(L)$ in \ExtrStoneFrm.
\end{lemma}
\begin{proof}
    Similarly to the general case, define \DEFSYM{vstar2}{$\tilde v_L$}$\colon \JI\Bo(L) \to L$ as
    $$ \tilde v_L\colon I \mapsto \bigvee I.$$

    From the Observation~\ref{p:boEQbc}, we know the definitions of $\tilde v_L$ and $v_L$ are the same, and $\tilde v_L$ is a frame isomorphism by~\ref{p:StoneFrmEquivalence}. From~\ref{p:completePartUnits}, $\tilde v_L$ is a morphism of \ExtrStoneFrm.
\end{proof}
...
\end{proposition}
\begin{proof}
    Let $L$ and $M$ be extremally disconnected Stone frames and let $f\colon L \to M$ be a basically complete frame homomorphism. The following diagram commutes
    \begin{diagram}
...
\num From \ref{p:BooleanizationFunctorOnExtrDSF}---\ref{p:JOBoIdentity}, we have a natural equivalence between the identity functor on \ComplBool{} resp. \ExtrStoneFrm{} and \Bo\JI{} resp. \JI\Bo{}.
% By the previous isomorphism lemmas we have the natural equivalence between identity functor and \Bo\JI{} resp. \JI\Bo{} and get an isomorphism of categories \ComplBool{} and \ExtrStoneFrm (two subcategories of \Frm).
As conclusion the functors \Bo{} and \JI{} are mutually inverse and we obtain the main result of this section
...
% TODO!
\num One may ask, if \Bo{} .... As $\Bo L$ is always a complete Boolean algebra and so $\JI\Bo(L)$ is extremally disconnected, there is no hope for non--extremally disconnected Stone frames.
