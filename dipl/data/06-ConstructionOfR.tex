...
\begin{proof}
    ...
    $\Leftarrow$ is straightforward and $\Rightarrow$ follows simply from $f(a^{**}) \leq f(a^{**})^{**} $. Commutativity of the diagram is precisely the equality $f(a^{**})^{**} = f(a)^{**}$.
...
In literature the frame homomorphisms satisfying (\oldref{e:2202iff20leq02}) are called \DEF{weakly open} and frame homomorphisms satisfying (\oldref{e:1001}) are called \DEF{nearly open}~\cite{banaschewski1994variants}.
...
\begin{proof}
    ...
    Denote by $\tilde i_B\colon B \to \Bo\JI(B)$ the mapping $a \mapsto \downset a$. From the previous Observation we know that the definitions of $\tilde i_B$ and $i_B$ coincide, therefore by~\ref{p:BoolEquivalence}, $\tilde i_B$ is a Boolean isomorphism. And from~\ref{p:completePartUnits}, we see that $\tilde i_B$ is a morphisms of \ComplBool{}.
